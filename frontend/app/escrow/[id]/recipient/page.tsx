...
export default function RecipientEscrowPage() {
  ...
  const loadEscrow = useCallback(async () => {
    ...
    try {
      ...
      if (data.recipient_address)
        setRecipientAddressInput(data.recipient_address);
      setError(null);
  ...
  const actorIsRecipient = escrow?.payee_user_id === actorUserId;
  ...
  async function handleSetRecipientAddress() {
    ...
    await withAction("set-recipient-address", async () => {
      const token = joinToken.trim();
      const address = recipientAddress.trim();
      if (!token) throw new Error("Join token is required.");
      if (!address) throw new Error("Recipient payout address is required.");
      const data = await setRecipientAddress(publicId, {
        join_token: token,
  ...
  async function handleServiceComplete() {
    if (!actorUserId) {
      setActionError("You must be signed in to mark service complete.");
      return;
    }
    await withAction("service-complete", async () => {
      const token = joinToken.trim();
      if (!token) throw new Error("Join token is required.");
      const data = await markServiceComplete(publicId, { join_token: token });
      setEscrow(data);
      void scanChain(false);
    });
  }
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
        ...
        {isLoaded && !isSignedIn && (
          <div className="bg-yellow-950/40 border border-yellow-800/30 rounded-lg p-4 mb-4">
            <p className="text-sm text-yellow-300">
              Sign in with Clerk to perform recipient actions.
            </p>
          </div>
        )}

        {!actorIsRecipient && escrow && (
          <div className="bg-yellow-950/40 border border-yellow-800/30 rounded-lg p-4 mb-4">
            <p className="text-sm text-yellow-300">
              Current actor is not the recipient for this escrow. Recipient
              actions may fail.
            </p>
          </div>
        )}
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          <section className="bg-neutral-900/60 backdrop-blur rounded-2xl p-5 border border-neutral-800">
            ...
            >
              {actionLoading === "service-complete"
                ? "Submitting..."
                : "Service Complete"}
            </button>

            <h3 className="text-sm font-semibold mt-5 mb-2">Payout Address</h3>
            <input
            ...
            >
              {actionLoading === "set-recipient-address"
                ? "Saving..."
                : "Set Recipient Address"}
            </button>

            <h3 className="text-sm font-semibold mt-5 mb-2">Start Dispute</h3>
            <input
              value={disputeReason}
            ...
            <button
              ...
              disabled={actionLoading !== null}
              className="mt-2 w-full bg-red-900 hover:bg-red-800 disabled:opacity-50 rounded-lg px-3 py-2 text-sm"
            >
              {actionLoading === "dispute" ? "Submitting..." : "Start Dispute"}
            </button>
          </section>
          ...
          <section className="bg-neutral-900/60 backdrop-blur rounded-2xl p-5 border border-neutral-800">
            ...
            <div className="grid sm:grid-cols-2 gap-y-2 text-sm">
              ...
              <p className="text-right">
                {escrow?.payer_user_id ? "Sender account claimed" : "Unclaimed"}
              </p>
              <p className="text-neutral-500">Recipient</p>
              <p className="text-right">
                {escrow?.payee_user_id
                  ? escrow.payee_user_id === actorUserId
                    ? "You (Recipient)"
          ...
          <div className="fixed inset-0 z-[70] bg-black/60 flex items-center justify-center p-4">
            <div className="w-full max-w-md bg-neutral-900/80 backdrop-blur border border-neutral-700 rounded-2xl p-5">
              ...
              <p className="text-sm text-neutral-300 mt-2">
                ...
                can now submit a review for the sender.
              </p>
