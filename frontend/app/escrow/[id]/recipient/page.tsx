...
export default function RecipientEscrowPage() {
  ...
  const loadEscrow = useCallback(async () => {
    ...
    try {
      ...
      setEscrow(data);
      if (data.recipient_address) setRecipientAddressInput(data.recipient_address);
      setError(null);
    ...
    <div className="min-h-screen bg-[#1d1d1d] text-white">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
        ...
        {!actorIsRecipient && escrow && (
          <div className="bg-yellow-950/40 border border-yellow-800/30 rounded-lg p-4 mb-4">
            <p className="text-sm text-yellow-300">
              Current actor is not the recipient for this escrow. Recipient actions may fail.
            </p>
          </div>
        )}
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          ...
          <section className="bg-neutral-900 rounded-xl p-5 border border-neutral-800">
            ...
            <div className="grid sm:grid-cols-2 gap-y-2 text-sm">
              ...
              <p className="text-right">{escrow?.payer_user_id ? "Sender account claimed" : "Unclaimed"}</p>
